<template>
  <ConfigSection title="连接" :readonly="readonly">
    <div class="field-row two-col">
      <ConfigField label="服务名称" type="text" v-model="form.serverName" prop="serverName"
        placeholder="要访问的代理名称" :readonly="readonly" />
      <ConfigField label="服务用户" type="text" v-model="form.serverUser"
        placeholder="留空表示相同用户" :readonly="readonly" />
    </div>
    <ConfigField label="密钥" type="password" v-model="form.secretKey"
      placeholder="共享密钥" :readonly="readonly" />
    <div class="field-row two-col">
